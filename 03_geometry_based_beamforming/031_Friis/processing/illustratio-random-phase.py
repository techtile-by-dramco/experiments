...
for i in range(100):
    ...
    h = np.exp(-1j * 2 * np.pi * np.random.random(size=d_EN.shape))
    ...
    # # plt.grid(True)
    # plt.tight_layout()
    # plt.savefig(f"../results/random/heatmap-dBm.png", bbox_inches="tight", transparent=True)
    # plt.show()
    ...
    # plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"../results/random/heatmap-uW-{i}.png", bbox_inches="tight", transparent=True)
    # plt.show()
