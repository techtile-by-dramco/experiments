...
void transmit_worker(size_t nsamps_per_buff, uhd::tx_streamer::sptr tx_stream,
                     size_t timeout, size_t num_channels, uhd::tx_metadata_t md, size_t num_requested_samples, float phase)
{
        std::vector<sample_fc32 *> buffs;

        sample_fc32 a = std::polar<float>(0.8, phase);
...
void recv_to_file(uint8_t channel_id, uhd::rx_streamer::sptr rx_stream,
                  ...
                  std::vector<size_t> rx_channel_nums)
{
        ...
        while (not stop_signal_called and (num_requested_samples > num_total_samps or num_requested_samples == 0))
        {
                ...
                std::memcpy(message.data(), (const char *)buff_ptrs[channel_id], num_bytes);
...
float start_cal(uint8_t id_cal, std::string serial, std::string server_ip, uhd::usrp::multi_usrp::sptr usrp, size_t num_channels, uhd::tx_streamer::sptr tx_stream, uhd::rx_streamer::sptr rx_stream, std::string otw, std::vector<size_t> rx_channel_nums, double rate, float bb_correction = float(0.0), double delay = 3.0)
{
        ...
        std::string rpl_45 = std::string(static_cast<char *>(reply.data()), reply.size());
        std::cout << "Current 45 phase: " << rpl_45 << "rad" << std::endl;

        float phase_diff = std::stof(rpl);
        // if (id_cal == 1)
        // {
        // phase_diff = std::stof(rpl);
        // }
        // else
        // {
        //         phase_diff = std::stof(rpl_45);
        // }
...
int UHD_SAFE_MAIN(int argc, char *argv[])
{
    ...
    ;
        ...
        if (vm.count("help"))
        {
                std::cout << "UHD TXRX Loopback " << desc << std::endl;
                return ~0;
        }
        ...
        uhd::usrp::multi_usrp::sptr usrp = uhd::usrp::multi_usrp::make(uhd::device_addr_t("mode_n=integer, int_n_step=1e6"));
        usrp->set_rx_dc_offset(false, uhd::usrp::multi_usrp::ALL_CHANS);
        // std::cout << "Creating the usrp device in NON INT mode args..." << std::endl;
        // uhd::usrp::multi_usrp::sptr usrp = uhd::usrp::multi_usrp::make(uhd::device_addr_t("mode_n=fractional"));
        std::cout
            << std::endl;
...
0.8
        ...
        float phi1 = start_cal(0, serial, server_ip, usrp, num_channels, tx_stream, rx_stream, otw, rx_channel_nums, tx_rate);
        ...
        while (!calibrated)
        {
                ...
                a_cal = start_cal(0, serial, server_ip, usrp, num_channels, tx_stream, rx_stream, otw, rx_channel_nums, tx_rate, -phi1);
                calibrated = (a_cal < 0.017 && a_cal > -0.017); // smaller than 1 degrees
                if (!calibrated)
                {
                        phi1 += a_cal;
                }
        ...
        // now syncing with external REF
        std::cout << "external REF" << std::endl;

        float phi2 = start_cal(1, serial, server_ip, usrp, num_channels, tx_stream, rx_stream, otw, rx_channel_nums, tx_rate);
        std::cout << std::endl;
        std::cout << std::endl;
        ...
        while (!stop_signal_called)
        {
                ...
                start_cal(1, serial, server_ip, usrp, num_channels, tx_stream, rx_stream, otw, rx_channel_nums, tx_rate, phi2 - phi1, 0.0);
                std::cout << std::endl;
                std::cout << std::endl;
        }
